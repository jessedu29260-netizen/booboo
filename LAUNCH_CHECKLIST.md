...
- ☑ **P0.1 · npm publish chain — DONE (2026-07-02).** All six live on the public registry: `@booboo-brain/spec·build·serve·viewer·cli@0.1.0` + `create-booboo@0.1.1` (0.1.0 deprecated — dead scope). Stage B clean-install gate re-run against the REAL registry: **ALL GREEN in 23s** (scaffold→install→build→REST→view→MCP in a clean node:20 container). Gotcha for posterity: a brand-new org scope takes ~15 min to propagate (404s on GET while `+ published` succeeds — don't panic-republish). Original dress rehearsal below.
...
- ☑ **P0.4 · Repo public + pushed — DONE (2026-07-02, Jesse go).** https://github.com/jessedu29260-netizen/booboo · first CI run green in 35s.
