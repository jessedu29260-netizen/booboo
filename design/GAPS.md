...
| C22 | **The Executive's dossier reads 0 reports** | 🔴 open | Its 36 monthly closes exist in the snapshot but are house-level (`cluster: null`), and the panel's dossier fetches reports by `cluster` — `ix.list` supports no `parent` filter. So rank II shows "0 reports filed" while holding three years of them. This is the same hollow-dossier class as C12 and should not be left: the fix is a `parent` filter on the panel API (or matching `data.agent` client-side off the full fetch `OrgScreen` already performs for health). |
