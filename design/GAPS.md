...
| **Engraved CARDS** (the plate itself, not the rail it hangs on) | 🔴 **over-claimed 2026-07-19, corrected same day** | I marked this ✅ having only built the rails and added an inset text-shadow to the name. The card is otherwise untouched: a 15px-radius rounded rect with a `hsl(hash(id))` per-agent hue gradient and an emoji avatar. Three objective violations of our own written law: (a) CRAFT §0 sets radii **2/3/6** — cards use 15px and 13px; (b) the cosmos ranks by **GOLD/SILVER/BRONZE**, the board ranks by hash-hue, so the two faces disagree on the one thing the one-ordering law says must match; (c) emoji is the primary visual token on 51 staff plates. Jesse's word for the result was "cheap" and he is right. This row stays 🔴 until the plate itself is engraved brass. |
