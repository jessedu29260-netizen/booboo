...
| **The left-to-right CASCADE** (master-key structure) | 🔴 **not built** | The material landed; the *structure* did not. The board is still the old top-down shape — GM centred above, department lanes stacked below — while the reference (and my own approved mockup, `board-mockup2.png`) is a left-to-right cascade: THE STANDARD → GM → departments → staff, each rank its own column, orthogonal brass elbows between them. The rank headers I added ("I THE HOUSE / II DEPARTMENTS…") currently label columns that **do not exist**, which is worse than having no headers. Next move. |
| Rank headers named, not inferred (ministry grammar) | 🟡 | Built and rendering, but see the row above — they do not yet correspond to real columns. |
