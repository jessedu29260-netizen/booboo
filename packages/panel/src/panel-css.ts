...
[data-theme="dark"] .bub { background: linear-gradient(168deg, var(--plate-lift), #180e16); }
...
[data-theme="dark"] .lane-tray { background: rgba(11, 5, 9, 0.4); }
...
.ag {
  ...
  width: calc(268px - min(var(--d, 0), 2) * 28px); /* 268 / 240 / 212 by rank */
  padding: 13px 15px; text-align: left;
  background: var(--bg-1);
  border: 1px solid var(--line); border-radius: 14px; cursor: grab;
  box-shadow: var(--sh-2);
...
/* rank IV reads lighter and rounder — a smaller bubble, not a different colour */
.ag.staff { border-radius: 11px; padding: 11px 13px; box-shadow: var(--sh-1); }
@keyframes settle { from { opacity: 0; transform: translateY(8px) scale(.985); } to { opacity: 1; transform: none; } }
/* the lift: bubbles rise toward the cursor, then settle. Slight, never showy. */
.ag:hover { transform: translateY(-2px); box-shadow: var(--sh-3); border-color: var(--line-2, #d9d3c9); }
...
/* health tints the bubble's wash — amber/red earn it, green stays quiet */
.ag.h-warn { border-color: #ecd9ae; background: linear-gradient(180deg, var(--warn-wash), var(--bg-1) 62%); }
.ag.h-fail { border-color: #edc9bf; background: linear-gradient(180deg, var(--fail-wash), var(--bg-1) 62%); }
...
.ag.root { cursor: default; width: 268px; padding: 15px 17px;
  background: linear-gradient(180deg, #fffdf8, var(--bg-1));
  border-color: #e8d9bd; box-shadow: var(--sh-3); }
...
.ag.staff .ag-name { font-size: 12px; font-weight: 600; letter-spacing: 0; text-transform: none; color: var(--ink-2); }
