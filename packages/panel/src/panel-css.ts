...
:root {
  /* The house tokens. Raw values are generated into app/tokens.css from
     design/tokens.json (scripts/gen-tokens.mjs); these are the panel's own
     semantic aliases over them, so the staff board, the cosmos and the site
     stay literally the same object. */
  --bg: #06080e;
  --bg-1: #0b0e15;
  --bg-2: #0f131c;
  --line: #1c2130;
  ...
  --accent: #c9a04a;
  --accent-2: #e8c877;
  --accent-dim: rgba(201, 160, 74, 0.13);
  --warn: #d6a23e;
...
.bar-brand { font-size: 17px; font-family: var(--display); letter-spacing: 0.01em; }
...
/* real vendor marks, tinted to the house so twelve logos read as one system.
   ...
   don't. */
.mac-brand {
  width: 13px; height: 13px; flex: 0 0 13px; display: block;
  filter: grayscale(1) brightness(0) invert(0.72) sepia(0.5) saturate(1.4) hue-rotate(3deg);
  opacity: .85; transition: filter .25s ease, opacity .25s ease, transform .25s ease;
}
.oc-mac:hover .mac-brand,
.oc-mac.sel .mac-brand { filter: none; opacity: 1; transform: scale(1.08); }
...
.oc-row.lanes > .oc-child {
  ...
  border-left: 2px solid hsl(var(--h) 45% 52% / 0.55);
  border-radius: 10px;
  background:
    linear-gradient(180deg, hsl(var(--h) 40% 50% / 0.05), transparent 60%),
    var(--bg-1);
  padding: 14px 16px 16px;
  transition: border-color .25s ease, box-shadow .35s ease, transform .35s ease;
}
.oc-row.lanes > .oc-child:hover {
  border-left-color: hsl(var(--h) 55% 62% / 0.9);
  box-shadow: 0 18px 44px -26px rgba(0,0,0,.95), 0 0 0 1px hsl(var(--h) 40% 50% / .18);
  transform: translateY(-1px);
}

/* the lane is a stage: cards sit ON it, so hovering one tilts it toward the
   viewer under the same key light the brass landmarks use in the cosmos. */
.oc-row.lanes > .oc-child > .ocn { perspective: 900px; perspective-origin: 30% 0%; }
.oc-row.lanes .ag {
  transform-style: preserve-3d;
  transition: transform .28s cubic-bezier(.2,.7,.25,1), box-shadow .28s ease, border-color .15s;
}
.oc-row.lanes .ag:hover {
  transform: translateZ(26px) translateY(-3px) rotateX(6deg);
  box-shadow:
    inset 0 1px 0 rgba(255,255,255,.09),
    inset 0 0 0 1px hsl(var(--h) 45% 50% / .18),
    0 2px 4px rgba(0,0,0,.55),
    0 26px 52px -22px hsl(var(--h) 60% 42% / .5),
    0 0 34px -16px hsl(var(--h) 70% 60% / .35);
}
.oc-row.lanes .ag:active { transform: translateZ(10px) translateY(-1px) rotateX(2deg); }
@media (prefers-reduced-motion: reduce) {
  .oc-row.lanes .ag:hover,
  .oc-row.lanes .ag:active { transform: none; }
