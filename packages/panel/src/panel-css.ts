...
/* plates keep their brass edge in light theme; the ground and ink swap. The old
   rule here tinted each plate by hash-hue — gone with the rest of the rainbow. */
[data-theme="light"] .ag {
  background: linear-gradient(168deg, #ffffff, #f3f0eb 60%);
  border-color: rgba(20, 40, 60, 0.12);
  box-shadow: 0 12px 26px -18px rgba(20, 40, 60, 0.4);
}
[data-theme="light"] .ag.root { background: linear-gradient(168deg, #fdf4e3, #f7ecd8); border-color: rgba(169, 117, 58, 0.4); }
...
.pnl-aurora {
  position: fixed; inset: -30%; pointer-events: none; z-index: 0; opacity: 0.75;
  background:
    radial-gradient(46% 52% at 14% 6%, rgba(217, 160, 91, 0.16), transparent 62%),
    radial-gradient(42% 48% at 92% 94%, rgba(139, 233, 176, 0.07), transparent 62%),
    radial-gradient(120% 110% at 50% 26%, #170b12 0%, transparent 74%);
...
.bar {
  ...
  background: rgba(11, 5, 9, 0.72); backdrop-filter: blur(10px);
  flex-wrap: wrap;
}
...
.oc-tray {
  ...
  background: rgba(11, 5, 9, 0.5);
  border: 1px solid var(--line-soft); border-top: 0; border-radius: 0 0 5px 5px;
...
.oc-mac {
  ...
  background: rgba(244, 238, 226, 0.03); border: 1px solid var(--line-soft);
...
.mac-brand {
  ...
  background: #fff; border-radius: 3px; padding: 3px; box-sizing: border-box;
  box-shadow: 0 4px 11px -5px rgba(0, 0, 0, 0.7);
...
.pnl-stars { position: fixed; inset: 0; z-index: 0; pointer-events: none; opacity: .45; width: 100%; height: 100%; }
...
.ledger-shelf {
  ...
  border: 1px solid var(--line); border-top: 2px solid var(--brass-deep);
...
.oc-row.lanes > .oc-child {
  ...
  border-left: 1px solid rgba(217, 160, 91, 0.22);
  border-radius: 5px;
  background: linear-gradient(180deg, rgba(217, 160, 91, 0.028), transparent 55%);
  padding: 15px 17px 17px;
  transition: border-color .25s ease, box-shadow .35s ease;
}
.oc-row.lanes > .oc-child:hover {
  border-left-color: rgba(217, 160, 91, 0.6);
  box-shadow: 0 18px 44px -26px rgba(0,0,0,.95), inset 0 0 0 1px rgba(217, 160, 91, .07);
}
@media (prefers-reduced-motion: reduce) {
...
.ag {
  ...
  width: calc(268px - min(var(--d, 0), 2) * 34px); /* 268 / 234 / 200 by rank */
  padding: 11px 13px; text-align: left;
  background: linear-gradient(168deg, var(--plate-lift), #180e16);
  border: 1px solid var(--line); border-radius: 4px; cursor: grab;
  box-shadow: 0 16px 34px -18px rgba(0, 0, 0, 0.85), inset 0 1px 0 rgba(244, 238, 226, 0.05);
  transition: border-color 0.15s, transform 0.15s, box-shadow 0.2s, opacity 0.15s;
  animation: settle 0.4s ease both;
}
/* the milled brass edge — the plate's fixing to the rail */
.ag::before {
  content: ""; position: absolute; left: 0; top: 0; bottom: 0; width: 2px;
  border-radius: 4px 0 0 4px; background: var(--brass-line);
  opacity: calc(1 - min(var(--d, 0), 2) * 0.28); /* dulls with rank */
}
@keyframes settle { from { opacity: 0; transform: translateY(6px); } to { opacity: 1; transform: none; } }
.ag:hover { border-color: rgba(217, 160, 91, 0.5); transform: translateY(-1px);
  box-shadow: 0 22px 44px -18px rgba(0, 0, 0, 0.9), 0 0 0 1px rgba(217, 160, 91, 0.18), inset 0 1px 0 rgba(244, 238, 226, 0.08); }
.ag.sel { border-color: var(--brass);
  box-shadow: 0 0 0 1px rgba(217, 160, 91, 0.4), 0 20px 44px -18px rgba(217, 160, 91, 0.3); }
.ag:focus-visible { outline: 2px solid var(--brass); outline-offset: 2px; }
.ag.over { border-color: var(--brass-hi); box-shadow: 0 0 0 3px var(--accent-dim), 0 12px 30px rgba(217, 160, 91, 0.22); transform: scale(1.02); }
.ag.dragging { opacity: 0.45; cursor: grabbing; }
/* health tints the plate's own edge — amber/red earn it, green stays quiet */
.ag.h-warn { border-color: rgba(217, 160, 91, 0.42); }
.ag.h-fail { border-color: rgba(224, 123, 106, 0.5); }

/* the lamp — a recessed indicator, top-right of the plate */
.ag-light { position: absolute; top: 11px; right: 12px; width: 6px; height: 6px; border-radius: 50%; }
.ag-light.ok { background: var(--ok); box-shadow: 0 0 8px rgba(139, 233, 176, 0.8); }
.ag-light.warn { background: var(--brass); box-shadow: 0 0 8px rgba(217, 160, 91, 0.9); }
.ag-light.fail { background: var(--fail); box-shadow: 0 0 9px rgba(224, 123, 106, 0.95); animation: pulse 1.4s ease-in-out infinite; }

/* rank I·II — the GM. Polished brass: the only plate that is lit rather than lit-upon. */
.ag.root { cursor: default; width: 268px; padding: 13px 15px;
  background: linear-gradient(168deg, #3a2a16, #1d1410);
  border-color: rgba(239, 195, 137, 0.4);
  box-shadow: 0 22px 48px -18px rgba(217, 160, 91, 0.34), inset 0 1px 0 rgba(244, 238, 226, 0.12); }
.ag.root .ag-name { font-size: 15px; color: var(--brass-hi); }
.ag.root::before { opacity: 1; width: 3px; }
...
.ag-ava {
  ...
  border-radius: 3px; background: rgba(217, 160, 91, 0.09);
  border: 1px solid rgba(217, 160, 91, 0.26);
...
.ag-name {
  ...
  text-shadow: 0 1px 0 rgba(0, 0, 0, 0.6), 0 -1px 0 rgba(244, 238, 226, 0.06);
}
/* staff plates: quieter, lighter weight — rank in the type as well as the metal */
.ag[style*="--d: 2"] .ag-name, .ag.staff .ag-name {
  font-size: 11.5px; font-weight: 500; letter-spacing: 0.05em; color: var(--ink-2); text-shadow: 0 1px 0 rgba(0, 0, 0, 0.5);
}
[data-theme="light"] .ag-name { text-shadow: none; }
.ag-role {
  font-family: var(--mono); font-size: 9.5px; line-height: 1.45; color: var(--ink-3);
...
.ag-facts {
  ...
  gap: 3px 10px; margin-top: 9px; padding-top: 8px;
...
.ag-fact-report { margin-left: auto; color: var(--ink-3); }
